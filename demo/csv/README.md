...
### `network-zones.csv` — 14 phân vùng mạng *(Sprint 23)*
...
| Environment | Zones |
|-------------|-------|
| PROD (DC) | `DMZ_PROD`, `APP_PROD`, `DB_PROD`, `INFRA_PROD` |
...
### `zone-ips.csv` — 23 IP entries *(Sprint 23)*
...
| Zone | Server codes |
|------|-------------|
| `DMZ_PROD` | SRV-PROD-GW-001 (10.31), SRV-PROD-LB-001 (10.41) |
| `APP_PROD` | SRV-PROD-APP-001 (10.11), APP-002 (10.12), APP-003 (10.71) |
| `DB_PROD` | SRV-PROD-DB-001 (10.21), DB-002 (10.22), DB-003 (10.81) |
| `INFRA_PROD` | SRV-PROD-CACHE-001 (10.51), MQ-001 (10.61) |
...
| `INFRA_DEV` | SRV-DEV-CACHE-001 (40.51) |
| `INTERNET` | 3 CIDR ranges: VN ISP, VN Mobile, VN Broadband |

---

### `firewall-rules.csv` — 57 rules *(Sprint 23)*
...
| Environment | ALLOW | DENY | PENDING | INACTIVE/REJECTED | Tổng |
|-------------|-------|------|---------|-------------------|------|
| PROD | 28 | 4 | 2 | 2 | 36 |
| UAT | 8 | 1 | 0 | 0 | 9 |
| DEV | 8 | 1 | 0 | 0 | 9 (có thêm 1 DENY cross-env) |

**Các luồng firewall chính:**
- `INTERNET → DMZ_PROD` — HTTP/HTTPS vào Load Balancer và Payment Gateway
- `DMZ_PROD → APP_PROD` — NGINX proxy đến IB Portal, Admin Portal
- `APP_PROD → APP_PROD` — IB API → CBS, CBS → TRAN gRPC, Payment flows
- `APP_PROD → DB_PROD` — App servers kết nối PostgreSQL (TCP 5432)
- `APP_PROD → INFRA_PROD` — Redis (6379) và RabbitMQ (5672)
- DENY rules — bảo vệ: cấm DEV/internet truy cập PROD DB, cấm truy cập trực tiếp CBS
