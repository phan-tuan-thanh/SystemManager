...
type ImportType = 'server' | 'network_zone' | 'zone_ip' | 'firewall';
...
// Dependency order: zones first, then zone IPs, then server (may ref zones), then firewall (refs server + zones)
      ...
      {phase === 'setup' && (
        <>
          <Alert
            ...
            description="Tải lên nhiều file CSV, chọn loại dữ liệu cho từng file, rồi bấm Bắt đầu nhập. Hệ thống tự động nhập theo thứ tự phụ thuộc: Network Zone → Zone IP → Server → Firewall Rules."
          />
          ...
          >
            ...
            <p className="ant-upload-hint">Hỗ trợ: Server, Network Zone, Zone IP, Firewall Rules</p>
          </Dragger>
